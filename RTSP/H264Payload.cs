...
namespace Rtsp
{
    ...
    public class H264Payload
    {
        ...
        {
            Trace.TraceInformation("RTP Data comprised of " + rtp_payloads.Count + " rtp packets");
            ...
            for (int payload_index = 0; payload_index < rtp_payloads.Count; payload_index++)
            {
                ...
                {
                    Trace.TraceInformation("Normal NAL");
                    norm++;
                ...
                {
                    Trace.TraceInformation("Agg STAP-A");
                    stap_a++;
                    ...
                    {
                        Trace.TraceInformation("H264 Aggregate Packet processing error");
                    }
                }
                else if (nal_header_type == 25)
                {
                    Trace.TraceInformation("Agg STAP-B not supported");
                    stap_b++;
                }
                else if (nal_header_type == 26)
                {
                    Trace.TraceInformation("Agg MTAP16 not supported");
                    mtap16++;
                }
                else if (nal_header_type == 27)
                {
                    Trace.TraceInformation("Agg MTAP24 not supported");
                    mtap24++;
                }
                else if (nal_header_type == 28)
                {
                    Trace.TraceInformation("Frag FU-A");
                    fu_a++;
                    ...
                    Trace.TraceInformation("Frag FU-A s=" + fu_header_s + "e=" + fu_header_e);
                ...
                {
                    Trace.TraceInformation("Frag FU-B not supported");
                    fu_b++;
                }
                else
                {
                    Trace.TraceInformation("Unknown NAL header " + nal_header_type + " not supported");
                }

            }

            // Output some statistics
            Trace.TraceInformation("Norm=" + norm + " ST-A=" + stap_a + " ST-B=" + stap_b + " M16=" + mtap16 + " M24=" + mtap24 + " FU-A=" + fu_a + " FU-B=" + fu_b);
