    ...
    public class RtspMessage : RtspChunk
    {
        ...
        public static RtspMessage GetRtspMessage(string aRequestLine)
        {
            ...
            if (requestParts.Length == 3)
            {
                ...
                {
                    Trace.TraceWarning( "Got a strange message {0}", aRequestLine);
        ...
        public void AddHeader(string line)
        {
            ...
            {
                Trace.TraceWarning( "Invalid Header received : -{0}-", line);
            }
        }
        ...
        public override void LogMessage(TraceLevel aLevel)
        {
            ...
            Trace.WriteLine(aLevel+ $"Commande : {Command}");
            foreach (KeyValuePair<string, string> item in _headers)
            {
                Trace.WriteLine(aLevel+ $"Header : {item.Key}: {item.Value}" );
            }

            if (Data.Length > 0)
            {
                Trace.WriteLine(aLevel+ $"Data :-{ASCIIEncoding.ASCII.GetString(Data)}-");
            }
        }
