...
namespace Rtsp
{
    ...
    public class RtspListener : IDisposable
    {
        ...
        private void DoJob()
        {
                    ...
                    if (currentMessage != null)
                    {
                        if (!(currentMessage is RtspData))
                        {
                            ...
                            if (currentMessage.SourcePort != null)
                                Trace.WriteLine( "Receive from {0}", currentMessage.SourcePort.RemoteAdress);
                        ...
                        if (currentMessage is RtspResponse)
                        {
                            ...
                            lock (_sentMessage)
                            {
                                ...
                                {
                                    Trace.TraceWarning( "Receive response not asked {0}", response.CSeq);
                                }
                            }
            ...
            {
                Trace.TraceWarning("IO Error", error);
                _stream.Close();
                _transport.Close();
            }
            catch (SocketException error)
            {
                Trace.TraceWarning("Socket Error", error);
                _stream.Close();
                _transport.Close();
            }
            catch (ObjectDisposedException error)
            {
                Trace.TraceWarning("Object Disposed", error);
            }
            catch (Exception error)
            {
                Trace.TraceWarning("Unknow Error", error);
//                throw;
            }
        ...
        public bool SendMessage(RtspMessage message)
        {
            ...
            if (!_transport.Connected)
            {
                ...
                Trace.TraceWarning("Reconnect to a client, strange !!");
                try
                {
                    Reconnect();
                }
            ...
            Trace.TraceInformation("Send Message");
            message.LogMessage();
                ...
                switch (currentReadingState)
                {
                    ...
                    case ReadingState.Data:
                        if (currentMessage.Data.Length > 0)
                        {
                            ...
                            Trace.TraceInformation( "Readen {0} byte of data", byteReaden);
                        }
        ...
        public void EndSendData(IAsyncResult result)
        {
            ...
            } catch (Exception e)
            {
                ...
                Trace.TraceInformation("Error during end send (can be ignored) " + e);
                result = null;
            }
        }
